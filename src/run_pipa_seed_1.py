...
source = ROOT / "data/raw/pipa_cave220.jpg"
output_dir = ROOT / "results/pipa_seed_1"
output_dir.mkdir(parents=True, exist_ok=True)

# 固定为 512×512，适配 SD 2 inpainting；原图始终保留，不会被覆盖。
reference = Image.open(source).convert("RGB").resize((512, 512))
mask = Image.new("L", (512, 512), 0)
ImageDraw.Draw(mask).rectangle((160, 192, 288, 320), fill=255)
...
result = pipe(
    prompt=(
        "an ancient Chinese Dunhuang cave mural, Dunhuang Cave 220 mural, musician playing pipa with halo, weathered mineral pigments, "
...
metadata = {
    "source": "Dunhuang mural Dunhuang Cave 220 mural, musician playing pipa with halo, weathered mineral pigmentsa; Wikimedia Commons public-domain source",
    "model": "sd2-community/stable-diffusion-2-inpainting",
    "mask": "central 128x128 square at coordinates (192, 192, 320, 320)",
    "seed": 20260929,
    "prompt": "ancient Chinese Dunhuang cave mural, Dunhuang Cave 220 mural, musician playing pipa with halo, weathered mineral pigments, weathered mineral pigments, historical fresco, highly detailed",
    "steps": 25,
    "guidance_scale": 7.5,
}
